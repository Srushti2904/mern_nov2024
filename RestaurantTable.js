...
const RestaurantTable = () => {
    ...
    const fetchRestaurants = async () => {
        try {
            const response = await axios.get("http://localhost:8002/getAllRestaurants");
    ...
    const handleDelete = async (id) => {
        await axios.delete("http://localhost:8002/deleteRecord/" + id);
        fetchRestaurants();
    }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (editRecord) {
            await axios.put("http://localhost:8002/updateRestaurant/"+form.id, form);
        } else {
            await axios.post("http://localhost:8002/insertData", form);
        }
